/**
* @file client.cpp
* @brief This file contains the code for creating a client which communicates to the client at port 8000, and sends the encoded word
*/
#include <bits/stdc++.h>
#include <stdio.h>
...
int main(int argc, char const *argv[])
{
    ...
    int dword[dsize];
    cout << "Enter bits separated by space\n";
    ...
    int err_size;
    int choice = 0;
    cout << "Enter\n1.Manually add errors\n2.Add a random error" << endl;
    cin >> choice;
    if (choice == 1)
    {
    ...
    else
    {
        ...
        int index = (double(random) / 10) * l;
        cout << "Flipping the " << index << " bit" << endl;
        df[index] = !df[index];
        for (j = l; j > 0; j--)
            cout << df[j] << " ";
        cout << endl;
